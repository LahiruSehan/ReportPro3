...
class ZohoLedgerApp {
  ...
  async handleOrgSwitch(orgId) {
    this.showLoading(20, "Re-Indexing Project...");
    ...
    this.hideLoading();
    this.log(`Project: ${this.state.currentOrgDetails?.name}`);
  }
  ...
  renderStatementUI() {
    ...
    const projectName = org.name || 'N/A';
    ...
    let content = `
      <div class="flex justify-between items-start mb-12">
        ...
        <div class="text-right flex-shrink-0">
          <h2 class="text-5xl font-black tracking-tighter leading-none text-indigo-600" contenteditable="true">STATEMENT</h2>
    ...
    this.state.selectedCustomerIds.forEach(id => {
      ...
      ledgerItems.forEach(item => {
        ...
        if (details && details.line_items) {
          groupedByRef[ref].lines.push(...details.line_items);
        } else {
          groupedByRef[ref].lines.push({ 
            name: `Aggregated ${item._type.toUpperCase()}`, 
            quantity: 1, 
            item_total: item.total || item.balance || 0 
          });
        }
      });
      ...
      sortedRefs.forEach(ref => {
        ...
        // Render Group Header (The Invoice Bar)
        content += `<tr class="invoice-group-header no-break">
        ...
        group.lines.forEach(li => {
          ...
          clientRunTotal += amt;
          content += this.createRowMarkup(li.name || li.description, li.quantity || 1, amt, ref, clientRunTotal, isCredit, group.info._type);
        });
      });
  ...
  downloadExcel() {
    this.showLoading(80, "Ultra-Detailed Data Export...");
    const data = [];
    const projectName = this.state.currentOrgDetails?.name || 'Unknown Project';
          ...
          if (details && details.line_items) {
            details.line_items.forEach(li => {
              data.push({
                ...
                'Date': r.date,
                'Reference': docRef,
                'Service / Item': li.name || '',
                'Description': li.description || '',
                'Qty': li.quantity || 1,
                'Rate': li.rate || 0,
                'Discount': li.discount || 0,
                'Tax Amount': li.tax_amount || 0,
                'Line Total': multiplier * (li.item_total || 0),
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Item_Ledger_Detailed");
    XLSX.writeFile(wb, `BizSense_InsightPRO_Detailed_${Date.now()}.xlsx`);
    this.hideLoading();
  }

  downloadPDF() {
    this.showLoading(85, "High-Definition PDF Generation...");
    
    // Create a pristine clone for PDF capture to avoid transform issues
    ...
    const opt = {
      margin: [10, 0, 10, 0],
      filename: `Detailed_Statement_${Date.now()}.pdf`,
  ...
  setZoom(val) {
    ...
    this.targets.renderArea.style.height = `${(standardA4H_px * this.state.zoom) + 100}px`;
  }
  ...
  autoFitZoom() {
    ...
    // Target 75% of the area to be filled
    const targetW = wrapperW * 0.75;
    const standardA4W_px = 21 * 37.8;
    const calculatedZoom = targetW / standardA4W_px;
    
    this.setZoom(Math.max(0.4, calculatedZoom));
  }
